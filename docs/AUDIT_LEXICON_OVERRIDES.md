...
The curated lexicon currently holds **9 entries**. All other words resolve via
the **rule engine** (src = `rule`). The only exceptions are words whose
pronunciation is lexical / loanword / proper-name idiosyncratic and **cannot**
be derived from orthography.
